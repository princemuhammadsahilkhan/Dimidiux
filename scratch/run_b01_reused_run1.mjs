/**
 * Step 11 — B01 REUSED Run 1 Only
 * Executes exactly ONE controlled run for Task B01 under the REUSED group using pinned capability cap_1788786148525_9mc4p v1.
 */
...
async function executeReusedRun1() {
  ...
  // 2. Ensure only BASELINE runs exist prior to executing REUSED Run 1
  let runsInStore = JSON.parse(memoryStore['evo_experiment_runs'] || '[]');
  runsInStore = runsInStore.filter((r) => r.group === 'BASELINE');
  memoryStore['evo_experiment_runs'] = JSON.stringify(runsInStore);

  let expsInStore = JSON.parse(memoryStore['evo_experiments'] || '[]');
  expsInStore = expsInStore.map((e) => e.id === experimentId ? { ...e, completedRuns: runsInStore.length } : e);
  memoryStore['evo_experiments'] = JSON.stringify(expsInStore);
  fs.writeFileSync(storeFile, JSON.stringify(memoryStore, null, 2), 'utf-8');
  ...
  console.log('Final Objective Plan Steps:', JSON.stringify(finalObj ? finalObj.plan : []));

  let runFixtureFolder = path.join(process.cwd(), runRecord.fixtureDirectory);
  let runFixtureFile = path.join(runFixtureFolder, 'ExperimentTest/hello.txt');
  if (!fs.existsSync(runFixtureFile)) {
    runFixtureFile = path.join(runFixtureFolder, 'hello.txt');
  }

  let rootFolder = path.join(process.cwd(), 'workspace/ExperimentTest');
  let rootFile = path.join(rootFolder, 'hello.txt');

  const checkFolder = fs.existsSync(runFixtureFolder) || fs.existsSync(rootFolder);
  const checkFile = fs.existsSync(runFixtureFile) || fs.existsSync(rootFile);
  let fileContents = null;
  if (fs.existsSync(runFixtureFile)) {
    fileContents = fs.readFileSync(runFixtureFile, 'utf-8').trim();
  } else if (fs.existsSync(rootFile)) {
    fileContents = fs.readFileSync(rootFile, 'utf-8').trim();
  }
  ...
  const physicalVerification = {
    ...
    overallVerified: checkFolder && checkFile && fileContents === 'Hello EVO'
  };

  console.log('\n====================================================');
  console.log('B01 REUSED RUN 1 EXECUTION REPORT');
  console.log('====================================================');
