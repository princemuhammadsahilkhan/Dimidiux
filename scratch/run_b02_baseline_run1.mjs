/**
 * Step 11 — B02 BASELINE Run 1 Only
 * Executes exactly ONE controlled run for Task B02 under the BASELINE group (capability reuse disabled).
 */
...
async function executeB02BaselineRun1() {
  ...
  // 3. Execute ONE run for BASELINE group (capability reuse explicitly disabled)
  ...
  let projectFilesFolder = path.join(runFixtureFolder, 'ProjectFiles');
  if (!fs.existsSync(projectFilesFolder) && fs.existsSync(path.join(runFixtureFolder, 'notes.txt'))) {
    projectFilesFolder = runFixtureFolder;
  }
  ...
  console.log('\n====================================================');
  console.log('B02 BASELINE RUN 1 EXECUTION REPORT');
  console.log('====================================================');
  ...
  console.log('Complete Verification Result:', JSON.stringify(physicalVerification, null, 2));
...
executeB02BaselineRun1().catch((err) => {
